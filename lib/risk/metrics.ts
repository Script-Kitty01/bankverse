...
export function evaluateThreshold(
  ...
  let trueNegative = 0;
  let falseNegative = 0;
  const rows = buildFeatureRows(transactions, priorHistory);
  ...
  return {
    ...
    falsePositiveRate:
      falsePositive / Math.max(1, falsePositive + trueNegative),
    expectedCostInr:
      falsePositive * costs.falsePositiveInr +
      falseNegative * costs.falseNegativeInr,
  };
}

export function trainDetector(
